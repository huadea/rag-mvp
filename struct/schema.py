...
FIELDS = [
    ...
    },
    {
        ...
        "field_type": "single",
        "group": "项目基础信息",
        "anchors": ["投标文件递交截止时间", "递交投标文件截止时间", "递交截止时间", "投标截止时间"],
    ...
    },
    {
        ...
        "anchors": ["招标范围", "采购范围"],
        "pattern": r"[:：]\s*(.+)",
        # 很多标书从不写"招标范围"这个词，只在标包划分处说"本项目共分
        # N个标包，分别为：xxx、xxx"，这里的"标包"是已验证有效的同义词，
        # 直接固化进静态同义词表，不必每次都指望 LLM 现场猜中它。
        "query_keywords": ["招标范围", "采购范围", "标包"],
    },
    {
        "field_name": "招标控制价",
        "field_type": "single",
